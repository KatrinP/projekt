...
def main():
    ...
    problem_size = 3
    search_space_ra = [[-5.12, 5.12] for i in range(problem_size)]
    search_space = [[-100, 100] for i in range(problem_size)]
    ...
    with open('results.txt', "a") as results_file:
        ...
        results_file.write("HMS: {}\n".format(HMS))
        results_file.write("Max_iter (function evaluation): {}\n".format(max_iter))
        results_file.write("HMCR: {}\n".format(HMCR))
        results_file.write("PAR: {}\n".format(PAR))
        results_file.write("PARmin (GHS): {}\n".format(PARmin))
        results_file.write("PARmax (GHS): {}\n".format(PARmax))
        results_file.write("Total iterations (nr of simualations): {}\n\n".format(total))
        results_file.write("****START OF THE RUN****\n\n")
        for file in lst:
            text_file = file + ".txt"
            ...
            with open(os.path.join('results',text_file), "a") as fout:
                ...
                for i in range(total):
                    ...
                    if "ra" in file:
                        best = eval(file).search(search_space_ra, max_iter, HMS, HMCR, PARmin, PARmax, problem_size, i)
                    else:
