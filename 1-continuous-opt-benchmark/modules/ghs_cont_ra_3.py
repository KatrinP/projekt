    ...
    for i in range(limit):
        if random.random() < HMCR:
            ...
            if random.random() < PAR:
                value = best["vector"][random.randint(0, limit-1)] + rand_in_bounds(-1.0, 1.0)
...
def search(search_space, max_iter, HMS, HMCR, PARmin, PARmax, problem_size, total):
    ...
    for i in range(max_iter):
        ...
        # write fitness
        if not os.path.exists(os.path.join(os.getcwd(), 'data', 'ghs_cont_ra_3')):
            os.mkdir(os.path.join(os.getcwd(), 'data', 'ghs_cont_ra_3'))
        path0 = os.path.join(os.getcwd(), 'data', 'ghs_cont_ra_3', 'fitness')
