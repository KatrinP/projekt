    ...
    for i in range(limit):
        if random.random() < HMCR:
            ...
            if random.random() < PAR:
                value = value + bw*rand_in_bounds(-1.0, 1.0)
...
def search(search_space, max_iter, HMS, HMCR, PAR, bw, problem_size, total):
    ...
    for i in range(max_iter):
        ...
        # write fitness
        if not os.path.exists(os.path.join(os.getcwd(), 'data', 'hs_cont_ra')):
            os.mkdir(os.path.join(os.getcwd(), 'data', 'hs_cont_ra'))
        path0 = os.path.join(os.getcwd(), 'data', 'hs_cont_ra', 'fitness')
